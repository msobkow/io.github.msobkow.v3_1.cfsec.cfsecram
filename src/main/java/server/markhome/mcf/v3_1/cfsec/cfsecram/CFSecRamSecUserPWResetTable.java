	...
	implements ICFSecSecUserPWResetTable
{
		...
		ICFSecSecUserPWReset iBuff )
	{
		...
		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset > subdictSentEMAddrIdx;
		if( dictBySentEMAddrIdx.containsKey( keySentEMAddrIdx ) ) {
			subdictSentEMAddrIdx = dictBySentEMAddrIdx.get( keySentEMAddrIdx );
		}
		else {
			subdictSentEMAddrIdx = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset >();
			dictBySentEMAddrIdx.put( keySentEMAddrIdx, subdictSentEMAddrIdx );
		}
		subdictSentEMAddrIdx.put( pkey, Buff );

		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset > subdictNewAcctIdx;
		if( dictByNewAcctIdx.containsKey( keyNewAcctIdx ) ) {
			subdictNewAcctIdx = dictByNewAcctIdx.get( keyNewAcctIdx );
		}
		else {
			subdictNewAcctIdx = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset >();
		...
		String SentToEMailAddr )
	{
		...
		if( dictBySentEMAddrIdx.containsKey( key ) ) {
			...
			Map< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset > subdictSentEMAddrIdx
				= new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset >();
		...
		ICFSecSecUserPWReset iBuff )
	{
		...
		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset > subdict;
		...
		if( dictBySentEMAddrIdx.containsKey( newKeySentEMAddrIdx ) ) {
			...
			subdict = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecUserPWReset >();
