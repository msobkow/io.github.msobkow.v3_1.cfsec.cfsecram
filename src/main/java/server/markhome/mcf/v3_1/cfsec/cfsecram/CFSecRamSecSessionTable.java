	...
	implements ICFSecSecSessionTable
{
		...
		ICFSecSecSession iBuff )
	{
		...
		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecSession > subdictSecUserIdx;
		if( dictBySecUserIdx.containsKey( keySecUserIdx ) ) {
			subdictSecUserIdx = dictBySecUserIdx.get( keySecUserIdx );
		}
		else {
			subdictSecUserIdx = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecSession >();
		...
		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecSession > subdictFinishIdx;
		if( dictByFinishIdx.containsKey( keyFinishIdx ) ) {
			subdictFinishIdx = dictByFinishIdx.get( keyFinishIdx );
		}
		else {
			subdictFinishIdx = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecSession >();
			dictByFinishIdx.put( keyFinishIdx, subdictFinishIdx );
		}
		subdictFinishIdx.put( pkey, Buff );

		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecSession > subdictSecProxyIdx;
		if( dictBySecProxyIdx.containsKey( keySecProxyIdx ) ) {
			subdictSecProxyIdx = dictBySecProxyIdx.get( keySecProxyIdx );
		}
		else {
			subdictSecProxyIdx = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecSession >();
		...
		ICFLibKeyHash256 SecUserId )
	{
		...
		if( dictBySecUserIdx.containsKey( key ) ) {
			...
			Map< $implCommaIJavaOptAtomType$, CFSecBuffSecSession > subdictSecUserIdx
				= new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecSession >();
		...
		ICFSecSecSession iBuff )
	{
		...
		Map< $implCommaIJavaOptAtomType$, CFSecBuffSecSession > subdict;
		...
		if( dictBySecUserIdx.containsKey( newKeySecUserIdx ) ) {
			...
			subdict = new HashMap< $implCommaIJavaOptAtomType$, CFSecBuffSecSession >();
